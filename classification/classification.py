...
metrics = pd.read_csv("/home/czhang/PycharmProjects/ModalityAI/ADL/mediapipe_pose_face/output_v3.csv")
...
for model_name, model in models.items():
    ...
    if model_name == "LogisticRegression":
        ...
        p_values = logit_model.pvalues[1:].tolist()  # Exclude intercept p-value

    # Save results in multiple rows per model
    ...
    for feature, coef, p_val in zip(X.columns, feature_importances_mean, p_values):
        results.append([model_name, accuracy, recall, f1, feature, coef, p_val])

# Save results in the new format
results_df = pd.DataFrame(results,
                          columns=["Model", "Accuracy", "Recall", "F1 Score", "Feature", "Coefficient", "P-Value"])
results_df.to_csv(
    "/home/czhang/PycharmProjects/ModalityAI/ADL/mediapipe_pose_face/mediapipe_pose_facetask.csv",
    index=False)
