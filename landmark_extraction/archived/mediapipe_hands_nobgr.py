...
def process_videos(input_dir, output_dir):
    ...
    hands = mp_hands.Hands(max_num_hands=2, min_detection_confidence=0.5,  # Customizable detection confidence
                    ...
                    for frame in tqdm(container.decode(stream), desc=f'Processing {video_file}'):
                        ...
                        if results.multi_hand_landmarks and results.multi_handedness:
                            for hand_index, (hand_landmarks, handedness) in enumerate(zip(results.multi_hand_landmarks, results.multi_handedness)):
                                ...
                                for point_idx, coordinates in points_to_draw.items():
                                    if point_idx == 'confidence_score':  # Skip confidence_score as it's not a coordinate
                                        continue
                                    x, y = coordinates
                                    cv2.circle(annotated_frame, (x, y), 5, (0, 255, 0), -1)
...
if __name__ == "__main__":
    input_directory = "/home/czhang/PycharmProjects/ModalityAI/ADL/recordings_crowdsourced"  # Change to your input directory
    output_directory = "/home/czhang/PycharmProjects/ModalityAI/ADL/crowd_source_mediapipe_hand"  # Output directory for annotations
