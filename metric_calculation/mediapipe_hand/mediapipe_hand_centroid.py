...
def compute_velocity_acceleration_jerk(wrist_positions, total_frames):
    ...
    if len(velocities) < 2:
        return avg_dist, avg_velocity, None, None, used_frame_count, percentage_frames_used

    accelerations = np.gradient(velocities)
    avg_acceleration = np.mean(accelerations)

    if len(accelerations) < 2:
        return avg_dist, avg_velocity, avg_acceleration, None, used_frame_count, percentage_frames_used

    jerks = np.gradient(accelerations)
    avg_jerk = np.mean(jerks)

    return avg_dist, avg_velocity, avg_acceleration, avg_jerk, used_frame_count, percentage_frames_used
...
def process_json_file(filepath, task_name, csv_writer):
    ...
    for audio_file, session_data in data.items():
        for video_file, frame_data in session_data.items():
            ...
            distance_left_hand = left_metrics[0] if left_metrics and left_metrics[0] is not None else np.nan
            distance_right_hand = right_metrics[0] if right_metrics and right_metrics[0] is not None else np.nan

            # Replace None with np.nan for averaging
            velocity_left_hand = left_metrics[1] if left_metrics and left_metrics[1] is not None else np.nan
            velocity_right_hand = right_metrics[1] if right_metrics and right_metrics[1] is not None else np.nan

            acceleration_left_hand = left_metrics[2] if left_metrics and left_metrics[2] is not None else np.nan
            acceleration_right_hand = right_metrics[2] if right_metrics and right_metrics[2] is not None else np.nan

            jerk_left_hand = left_metrics[3] if left_metrics and left_metrics[3] is not None else np.nan
            jerk_right_hand = right_metrics[3] if right_metrics and right_metrics[3] is not None else np.nan

            # Compute averages of metrics across both hands (ignoring NaN values)

            avg_dist_both_hands = np.nanmean([distance_left_hand, distance_right_hand]) if not (
                    np.isnan(distance_left_hand) and np.isnan(distance_right_hand)) else None
            ...
            if avg_dist_both_hands is None and avg_velocity_both_hands is None and avg_acceleration_both_hands is None and avg_jerk_both_hands is None:
                continue
            ...
            higher_dist_metric = distance_left_hand if dominant_hand == "Left" else distance_right_hand
...
if __name__ == "__main__":
    ...
    #     "/home/czhang/PycharmProjects/ModalityAI/ADL/crowd_source_mediapipe_hand_results/centroid/metrics_face.csv"
    # )  # Replace with your actual directory path

    main(
        "/home/czhang/PycharmProjects/ModalityAI/ADL/mediapipe_hand_no_bgr_confidence_score",
        "/home/czhang/PycharmProjects/ModalityAI/ADL/mediapipe_hand_nobgr_results/centroid/metrics_face.csv"
    )
