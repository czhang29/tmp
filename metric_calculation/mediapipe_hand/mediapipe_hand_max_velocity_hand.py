...
def get_wrist_positions(data, hand):
    ...
    for frame, annotations in data.items():
        if annotations.get(hand):  # Check if the specified hand exists in the frame
            wrist_positions[int(frame)] = annotations[hand].get("0", [])  # Assuming "0" corresponds to the wrist position
...
def process_json_file(filepath, task_name, csv_writer):
    ...
    for audio_file, session_data in data.items():
        for video_file, frame_data in session_data.items():
            ...
            left_total_frames = max(left_wrist_positions.keys(), default=0) - min(left_wrist_positions.keys(), default=0) + 1
            right_total_frames = max(right_wrist_positions.keys(), default=0) - min(right_wrist_positions.keys(), default=0) + 1
...
if __name__ == "__main__":
    main(
        ...
        "/home/czhang/PycharmProjects/ModalityAI/ADL/mediapipe_hand_nobgr_results/max_velocity_hand/metrics_face.csv"
