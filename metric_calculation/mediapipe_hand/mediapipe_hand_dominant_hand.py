...
def compute_velocity_acceleration_jerk(wrist_positions, total_frames):
    ...
    if len(velocities) < 2:
        return avg_distance, avg_velocity, None, None, used_frame_count, percentage_frames_used

    accelerations = np.gradient(velocities)
    avg_acceleration = np.mean(accelerations)

    if len(accelerations) < 2:
        return avg_distance, avg_velocity, avg_acceleration, None, used_frame_count, percentage_frames_used

    jerks = np.gradient(accelerations)
    avg_jerk = np.mean(jerks)

    return avg_distance, avg_velocity, avg_acceleration, avg_jerk, used_frame_count, percentage_frames_used
        ...
        for video_file, frame_data in session_data.items():
            ...
            if metrics_to_report and metrics_to_report[0] is not None:  # Ensure valid metrics exist
                csv_writer.writerow([
                    ...
                    metrics_to_report[0],  # Velocity
                    metrics_to_report[1],  # Acceleration
                    metrics_to_report[2],  # Jerk
                    metrics_to_report[3]  # Jerk
                ])
...
if __name__ == "__main__":
    ...
    #     "/home/czhang/PycharmProjects/ModalityAI/ADL/crowd_source_mediapipe_hand_results/centroid/dominant_hand/dominant_hand_metrics_face.csv"
    # )

    main(
        "/home/czhang/PycharmProjects/ModalityAI/ADL/mediapipe_hand_no_bgr_confidence_score",
        "/home/czhang/PycharmProjects/ModalityAI/ADL/mediapipe_hand_nobgr_results/centroid/dominant_hand/dominant_hand_metrics_face.csv"
    )
