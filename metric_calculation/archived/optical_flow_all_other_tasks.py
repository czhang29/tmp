...
hair_output_dir = "/home/czhang/PycharmProjects/ModalityAI/ADL/optical_flow_hair/"
teeth_output_dir = "/home/czhang/PycharmProjects/ModalityAI/ADL/optical_flow_teeth/"

# Ensure output directories exist
os.makedirs(hair_output_dir, exist_ok=True)
os.makedirs(teeth_output_dir, exist_ok=True)

# Iterate over task folders containing 'Hair' or 'Teeth' in the name
for task_name in os.listdir(mediapipe_dir):
    if "Hair" in task_name or "hair" in task_name:
        output_task_path = os.path.join(hair_output_dir, task_name)
    elif "Teeth" in task_name or "teeth" in task_name:
        output_task_path = os.path.join(teeth_output_dir, task_name)
    else:
        continue
    ...
    for json_file in os.listdir(task_path):
        ...
        # Check if there are valid Mediapipe landmarks
        has_valid_landmarks = any(
            any(landmarks for landmarks in data[audio_key][video_key].values())
            for audio_key in data for video_key in data[audio_key]
        )
        if not has_valid_landmarks:
            continue  # Skip session if no valid landmarks
